...
export default function (editedPath: string, inputConfig: Config) {
    ...
    const onPropChange = (prop: string, newVal: RuleValue) => setEditedObj((obj) => ({ ...obj, [prop]: newVal }));

    const onDeleteProp = (prop: string) => setEditedObj((obj) => {
    ...
    const outObjEntries = Object.entries(editedObj)
        .map(([prop, val]) => (prop === "tags" && val.length === 1 ? ["tag", val[0]] : [prop, val]))
        .filter(([_, val]) => val !== "" && val !== null);
    const outObj = Object.fromEntries(outObjEntries);
    const outConf = editedColIdx === null
        ? {
            ...inputConfig,
            [editedKey]: outObj,
        }
    ...
    return {
        yamlConfig, onPropChange, onDeleteProp, editedObj,
    };
}
