...
import { executeUpdateQuery, getAllNotebooks, getAllTags, getConfigNote, searchNotes, setConfigNote } from "./noteData";
...
async showConfigUI(targetPath: string) {
  ...
  async postInsertNoteToColumn(msg: InsertNoteToColumnAction) {
    ...
    let noteData = await this.joplinService.getNoteDataById(noteId);
...
async handleKanbanMessage(msg: Action) {
    ...
    return this.kanbanMessageQueue.enqueue(async () => {
        return this.handleQueuedKanbanMessage(msg);
    }).catch(
      () => {});
  }
