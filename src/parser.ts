...
export const validateConfig = (config: Config | {} | null): Message | null => {
    if (!config || typeof config !== "object") return configErr("Configuration is empty");
    if (!("columns" in config)) return configErr("There are no columns defined!");
    if (!Array.isArray(config.columns)) return configErr("Columns has to be a list");
    if (config.columns.length === 0) return configErr("You have to define at least one column");

    if ("filters" in config) {
        if (typeof config.filters !== "object" || Array.isArray(config.filters)) return configErr("Filters has to contain a dictionary of rules");
        for (const key in config.filters) {
            if (!(key in rules) && key !== "rootNotebookPath") return configErr(`Invalid rule type "${key}" in filters`);
        }
    }

    if ("sort" in config) {
        if (typeof config.sort !== "object" || config.sort.by === undefined) return configErr("Sort must be a dictionary with a single 'by' field");
        let cs = config.sort.by;
        if (cs.startsWith("-")) cs = cs.substring(1);
        if (["createdTime", "title"].indexOf(cs) === -1) return configErr("Sort must be one of 'createdTime', 'title'; optionally prefix by '-' for descending order");
    }
    ...
    for (const col of config.columns) {
        ...
        for (const key in col) {
            if (!(key in rules) && !["backlog", "name", "newNoteTitle", "link"].includes(key)) return configErr(`Invalid rule type "${key}" in column "${col.name}"`);
            if (isBacklog && !["backlog", "name", "newNoteTitle", "link"].includes(key)) {
                return configErr(
